...
def get_location_codes():
    ...
    try:
        ...
        if cached_data:
            ...
            df_locations = pd.read_json(cached_data)
            print(f"Retrieved {len(df_locations)} locations from cache")
            return df_locations
...
def fetch_data():
    ...
    if now_est.time() < time(11, 0) or now_pst.time() > time(23, 0):
        logger.info("Outside operating hours; not fetching new data")
        return pd.DataFrame({
            'error': ["Stores are currently closed"],
            'location': ['Unknown'],
    ...
    try:
        ...
        logger.info(f"After $0 filter DataFrame shape: {df.shape}")
        
        if df.empty:
            logger.warning("Warning: DataFrame empty after filtering $0 sales")
            return pd.DataFrame({
        ...
        if not all(col in df.columns for col in columns[:-1]):  # Exclude refresh_time from check
            logger.warning(f"Warning: Missing columns: {df.columns}")
        
        # Cache data for 15 min
        cached_json = df[columns].to_json()
        if isinstance(cache, redis.Redis):
            cache.set(cache_key, cached_json.encode('utf-8'), ex=900)
        else:
